...
public class VentanaPrincipal extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evento) {
        if (evento.getSource() == calcular) { 
            ...
            JTextField[] np = {campoNota1, campoNota2, campoNota3, campoNota4, campoNota5};
            for (int i = 0; i < 5; i++) {
                try {
                    notas.listaNotas[i] = Double.parseDouble(np[i].getText());
                } catch (NumberFormatException e) {
                    JOptionPane.showMessageDialog(null, "Error: Ingrese solo números en los campos de texto.");
                    return;
                }
            }
            if (notas.listaNotas[0] == 0 || notas.listaNotas[1] == 0 || notas.listaNotas[2] == 0 || notas.listaNotas[3] == 0 || notas.listaNotas[4] == 0) {
                JOptionPane.showMessageDialog(null, "Error: Ingrese todas las notas.");
                return;
            }
            ...
            desviacion.setText("Desviación estándar = " + String.valueOf(String.format("%.2f", desv)));
